...
const emailValidation = (email, res) => {
    if (!email || email.length === 0) return res.status(400).json({"message": "O campo \"email\" é obrigatório"});

    if (!email.match(emailRegex)) return res.status(400).json({"message": "O \"email\" deve ter o formato \"email@email.com\""});
}

const passwordValidation = (password, res) => {
    if (!password || password === 0) return res.status(400).json({"message": "O campo \"password\" é obrigatório"});
